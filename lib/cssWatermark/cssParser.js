...
CssParser.prototype.parse = function (code, options) {
    ...
    for (let i = 0; i < cssObject.stylesheet.rules.length; ++i) {
        ...
        if (cssObject.stylesheet.rules[i].type !== 'charset') {
            declarations = [];
            ...
            newRule = new Rule(oldRule.type, oldRule.selectors, declarations);
            rules.push(newRule);
        }
    }
...
const Rule = function (type, selectors, declarations) {
...
Rule.prototype.compare = function (rule) {
    ...
    return this.toString().localeCompare(rule.toString());
};
...
Rule.prototype.toString = function () {
    ...
    if (this.selectors !== undefined && this.selectors.length !== 0) {
        str = this.selectors[0];
        for (let i = 1; i < this.selectors.length; ++i) {
            str = str + ',' + this.selectors[i];
        }
    }
    else {
        str = this.type + JSON.stringify(this.declarations);
    }
    return str;
};
