...
const epubUploadHandler = function (request, reply) {
    ...
    const outputerr = Fs.createWriteStream('./stderr.log');
    const logger = new Console(output, outputerr);
    ...
    uploadedBook.on('end', () => {
        ...
        logger.log('Upload end : data size ' + buffer.length);
...
const epubHandler = function (request, reply) {
    ...
    tempFiles.getFile(epubID, (err, data) => {
        ...
        if (err) {
            ...
            epub.on('end', () => {
                ...
                metadata.id = epubID;
                metadata.image = epub.getCoverImageID();
...
exports.epubHandler = epubHandler;
